...
def find_hitting_time_inhomogenous(t, y, propensity_function, rng):
    hitting_point = rng.exponential(1)
    f = inhomogeneous_upper_bound_f_factory(t, y, hitting_point, propensity_function)
...
def expand_step_with_t_eval(t, y, hitting_time, update, t_eval, t_end):
    ...
    y_history = np.zeros((len(y), len(t_history)))
    y_history[:,:len(t_history)-1] = y
    y_history[:,len(t_history)-1]  = y+update

    return t_history, y_history

def gillespie_step(t, y, t_end, N, propensity_function, rng, t_eval):
    hitting_time = find_hitting_time_inhomogenous(t, y, propensity_function, rng).tau
    endpoint_propensities = propensity_function(t+hitting_time, y)
    ...
    t_history, y_history = expand_step_with_t_eval(t,y,hitting_time,update,t_eval,t_end)
...
def homogeneous_gillespie_step(t, y, t_end, N, propensity_function, rng, t_eval):
    ...
    if t + hitting_time > t_end:
        ...
        return Step(*expand_step_with_t_eval(t,y,t_end-t,update,t_eval,t_end), GillespieStepStatus.t_end)
...
class GillespieSimulator(Simulator):
    def step(self, t, y, t_end, rng, t_eval):
        if self.inhomogeneous:
            return gillespie_step(t, y, t_end, self.N, self.propensity_function, rng, t_eval) 
        else:
